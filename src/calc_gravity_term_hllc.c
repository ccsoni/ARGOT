                     ...
                     struct run_param  *this_run, int axis)
{
  ...
  cs_R = sqrt(gamma_R*pres_R/dens_R);
    ...
    float pres_pvrs = 0.5*(pres_L+pres_R) - 
      0.5*(vel_R-vel_L)*0.5*(dens_L+dens_R)*0.5*(cs_L+cs_R);
    pres_star = fmax(0.0, pres_pvrs);
#endif
#ifdef __TWO_RAREFACTION__ 
    /* two-rarefaction riemann solver */
    float zindx = 0.5*(gamm1_i)/gamma_i;
    pres_star = pow((cs_L+cs_R-0.5*gamm1_i*(vel_R-vel_L))/(cs_L/pow(pres_L,zindx) + cs_R/pow(pres_R,zindx)),1.0/zindx);
#endif
    ...
    pres_star = (gL*pres_L + gR*pres_R - (vel_R-vel_L))/(gL+gR);
#endif
  ...
  SL = fmin(vel_L - cs_L*qL, vel_R - cs_R*qR);
  SR = fmax(vel_R + cs_R*qR, vel_L + cs_L*qL);
  Sstar = (pres_R-pres_L + dens_L*vel_L*(SL-vel_L) - dens_R*vel_R*(SR-vel_R))/(dens_L*(SL-vel_L)-dens_R*(SR-vel_R));
  
  /* numerical flux */
  if(0.0<=SL){
    *flux_plus = dens_L*vel_L;
  }else if(SL<0.0 && 0.0<=Sstar) {
    *flux_plus = Sstar*(SL*dens_L - dens_L*vel_L)/(SL-Sstar);
  }else if(Sstar < 0.0 && 0.0 <=SR) {
    *flux_plus = Sstar*(SR*dens_R - dens_R*vel_R)/(SR-Sstar);
  }else if(SR<0.0) {
    *flux_plus = dens_R*velz_R;
  }
  ...
  /* numerical flux */
  if(0.0<=SL){
    *flux_minus = dens_L*vel_L;
  }else if(SL<0.0 && 0.0<=Sstar) {
    *flux_minus = Sstar*(SL*dens_L - dens_L*vel_L)/(SL-Sstar);
  }else if(Sstar < 0.0 && 0.0 <=SR) {
    *flux_minus = Sstar*(SR*dens_R - dens_R*vel_R)/(SR-Sstar);
  }else if(SR<0.0) {
    *flux_minus = dens_R*velz_R;
  }
