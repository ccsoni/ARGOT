	...
	for(ix=0;ix<NMESH_X_LOCAL;ix++) {
	  ...
	  for(iy=0;iy<NMESH_Y_LOCAL;iy++) {
	    ...
	    for(iz=0;iz<NMESH_Z_LOCAL;iz++) {
	      ...
	      float dens_lo, dens_hi;
	      float uene_lo, uene_hi;
	      float eneg_lo, eneg_hi;

	      tgt = &MESH(ix,iy,iz);

	      dens_lo = nH/this_run.denstonh;
	      dens_hi = 0.125*dens_lo;

	      uene_lo = tmpr/(this_run.uenetok*WMOL(ioneq_chem));
	      eneg_lo = dens_lo*uene_lo;

	      eneg_hi = 0.1*eneg_lo;
	      uene_hi = eneg_hi/dens_hi;

	      if(xpos+ypos < this_run.xmin+0.8*(this_run.xmax-this_run.xmin)) {
		tgt->dens = dens_lo;
		tgt->momx = 0.0;
		tgt->momy = 0.0;
		tgt->momz = 0.0;
