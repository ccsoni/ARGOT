...
int main(int argc, char **argv)
{
  ...
  for(rank_x=0;rank_x<NNODE_X;rank_x++) {
	...
	for(int ix=0;ix<NMESH_X_LOCAL;ix++) {
	  int jx = NMESH_X_LOCAL*this_run.rank_x + ix;
	  for(int iy=0;iy<NMESH_Y_LOCAL;iy++) {
	    int jy = NMESH_Y_LOCAL*this_run.rank_y + iy;
	    for(int iz=0;iz<NMESH_Z_LOCAL;iz++) {
	      int jz = NMESH_Z_LOCAL*this_run.rank_z + iz;
	      struct fluid_mesh *tgt;

	      tgt = &MESH(ix,iy,iz);

#if 1
	      if(jx==NMESH_X_TOTAL/2 && 
		 jy==NMESH_Y_TOTAL/8 && 
		 jz==NMESH_Z_TOTAL/2) {
		tgt->dens = 1.0/(this_run.delta_x*this_run.delta_y*this_run.delta_z);
	      }else{
		tgt->dens = 0.0;
	      }
#else
	      tgt->dens = (rank_x+1.0)/(this_run.delta_x*this_run.delta_y*this_run.delta_z);
#endif
	      tgt->momx = 0.0;
	      tgt->momy = 0.0;
	      tgt->momz = 0.0;
