		   ...
		   struct run_param *this_run)
{
  ...
  for(ix=0;ix<NMESH_X_LOCAL;ix++) {
    ...
    for(iy=0;iy<NMESH_Y_LOCAL;iy++) {
      ...
      //unit scale
      float uene = tgt->uene;
      if(uene<1.0e-5) uene=1.0e-5; 
      float kene = 0.5*NORML2(tgt->momx,tgt->momy,tgt->momz)/tgt->dens; 
      float pene = -0.5*tgt->pot*tgt->dens;
      float tene = uene*tgt->dens;
      float etrp_func = (gamma-1.0)*uene*tgt->dens/pow(tgt->dens,gamma);

      float vel_norm = sqrt(NORML2(tgt->momx,tgt->momy,tgt->momz))/tgt->dens;
      float cs = sqrt((gamma-1.0)*gamma*uene);
      float mach_num = vel_norm/cs;
      float pres = tgt->dens*SQR(cs);
      ...
      map[1][jx][jy] = log10(uene*this_run->uenetok*wmol);  //tmpr
      map[2][jx][jy] = log10(uene);                         //uene //unit scale
      //      map[3][jx][jy] = log10(tene); 
      map[3][jx][jy] = log10(pres);
      ...
      map[8][jx][jy] = log10(mach_num); 

      /*
      printf("%e %e %e %e %e %e %e %e %e\n",
	     map[0][jx][jy],map[1][jx][jy],map[2][jx][jy],map[3][jx][jy],
	     map[4][jx][jy],map[5][jx][jy],map[6][jx][jy],map[7][jx][jy],
	     map[8][jx][jy]);
      */
    }
  }
...
int main(int argc, char **argv) 
{
  ...
  */
  if(argc < 6) {
    fprintf(stderr, 
	    "Usage: %s <X|Y|Z> <slice position> <<vmin> <vmax>|<-file> <scale_list>>  <input prefix 1> <input prefix 2> <input prefix ...>\n",
	    argv[0]);
    exit(EXIT_FAILURE);
  }
  ...
  float slice_pos;
  PLFLT **map[file_num][MN];
  PLFLT **vec[file_num][2]; 
  PLFLT *pos1, *pos2;


  slice_pos = atof(argv[2]);

  for(int fn=0; fn<file_num; fn++) {
    
    for(int mn=0; mn<MN; mn++) {
      if(strcmp(argv[1],"X") == 0) {
	plAlloc2dGrid(&map[fn][mn],NMESH_Y_TOTAL,NMESH_Z_TOTAL);
      }else if(strcmp(argv[1],"Y") == 0) {
	plAlloc2dGrid(&map[fn][mn],NMESH_X_TOTAL,NMESH_Z_TOTAL);
      }else if(strcmp(argv[1],"Z") == 0) {
	plAlloc2dGrid(&map[fn][mn],NMESH_X_TOTAL,NMESH_Y_TOTAL);
      }else{
    ...
    if(strcmp(argv[1],"X") == 0) {
      plAlloc2dGrid(&vec[fn][0],NMESH_Y_TOTAL,NMESH_Z_TOTAL);
      plAlloc2dGrid(&vec[fn][1],NMESH_Y_TOTAL,NMESH_Z_TOTAL);
    }else if(strcmp(argv[1],"Y") == 0) {
      plAlloc2dGrid(&vec[fn][0],NMESH_X_TOTAL,NMESH_Z_TOTAL);
      plAlloc2dGrid(&vec[fn][1],NMESH_X_TOTAL,NMESH_Z_TOTAL);
    }else if(strcmp(argv[1],"Z") == 0) {
      plAlloc2dGrid(&vec[fn][0],NMESH_X_TOTAL,NMESH_Y_TOTAL);
      plAlloc2dGrid(&vec[fn][1],NMESH_X_TOTAL,NMESH_Y_TOTAL);
    }else{
      fprintf(stderr, "Invalid slice direction\n");
      exit(EXIT_FAILURE);
    }
  ...
  for(int fn=0; fn<file_num; fn++) {
	...
	for(rank_z=0;rank_z<NNODE_Z;rank_z++) {
	  ...
	  sprintf(filename, "%s_%03d_%03d_%03d", 
		  argv[fargc], rank_x, rank_y, rank_z);
	  ...
	  if(strcmp(argv[1],"X") == 0 &&
	    ...
	    compile_map_x(mesh, map[fn], vec[fn],
	    ...
	    compile_map_y(mesh, map[fn], vec[fn], 
			  pos1, pos2, slice_pos, &this_run);
	    ...
	    input_mesh_single(mesh, &this_run, filename);
	    fclose(fp);
	    compile_map_z(mesh, map[fn], vec[fn],
			  pos1, pos2, slice_pos, &this_run);
  ...
  for(int mn=0; mn<MN; mn++) {
	...
	for(imesh=0;imesh<NMESH_Y_TOTAL;imesh++) {
	  for(jmesh=0;jmesh<NMESH_Z_TOTAL;jmesh++) {
	    fmin[mn] = MIN(map[fn][mn][imesh][jmesh], fmin[mn]);
	    fmax[mn] = MAX(map[fn][mn][imesh][jmesh], fmax[mn]);
	  }
	}
      }else if(strcmp(argv[1],"Y")==0) {
  ...
  if(strcmp(argv[3],"-file")==0) {
    ...
    for(int mn=0; mn<MN; mn++) {
      ...
      if(atof(argv[3])==0.0 && atof(argv[4])==0.0 ) {
	get_minmax(fmin[mn], fmax[mn], &vmin[mn], &vmax[mn]);
	
    }else{
	fmin[mn] = vmin[mn] = atof(argv[3]);
	fmax[mn] = vmax[mn] = atof(argv[4]);
      }
    }
  ...
  for(int fn=0; fn<file_num; fn++) {
    ...
    for(int mn=0; mn<MN; mn++) {
      ...
      if(strcmp(argv[1],"X")==0) {
	view_map(map[fn][mn], pos1, pos2, NMESH_Y_TOTAL, NMESH_Z_TOTAL, 
		 1, NMESH_Y_TOTAL, 1, NMESH_Z_TOTAL, vmin[mn], vmax[mn], 
		 plot_title[mn], "Y", "Z", "log",mn);
	
	plFree2dGrid(map[fn][mn],NMESH_Y_TOTAL,NMESH_Z_TOTAL);
      
      }else if(strcmp(argv[1],"Y")==0) {
	view_map(map[fn][mn], pos1, pos2, NMESH_X_TOTAL, NMESH_Z_TOTAL, 
		 1, NMESH_X_TOTAL, 1, NMESH_Z_TOTAL, vmin[mn], vmax[mn],
		 plot_title[mn], "X", "Z", "log",mn);
	
	plFree2dGrid(map[fn][mn],NMESH_X_TOTAL,NMESH_Z_TOTAL);
	
      }else if(strcmp(argv[1],"Z")==0) {
	view_map(map[fn][mn], pos1, pos2, NMESH_X_TOTAL, NMESH_Y_TOTAL, 
		 1, NMESH_X_TOTAL, 1, NMESH_Y_TOTAL, vmin[mn], vmax[mn], 
		 plot_title[mn], "X", "Y", "log",mn);
	
	plFree2dGrid(map[fn][mn],NMESH_X_TOTAL,NMESH_Y_TOTAL);
    ...
    /* plot vector */
    if(strcmp(argv[1],"X")==0) {
      view_vec(vec[fn], NMESH_Y_TOTAL/VS, NMESH_Z_TOTAL/VS, 1, NMESH_Y_TOTAL/VS, 1, NMESH_Z_TOTAL/VS,
	       "vec", "Y", "Z");
      plFree2dGrid(vec[fn][0],NMESH_Y_TOTAL,NMESH_Z_TOTAL);
      plFree2dGrid(vec[fn][1],NMESH_Y_TOTAL,NMESH_Z_TOTAL);
    }else if(strcmp(argv[1],"Y")==0) {
      view_vec(vec[fn], NMESH_X_TOTAL/VS, NMESH_Z_TOTAL/VS, 1, NMESH_X_TOTAL/VS, 1, NMESH_Z_TOTAL/VS,
	       "vec", "X", "Z");
      plFree2dGrid(vec[fn][0],NMESH_X_TOTAL,NMESH_Z_TOTAL);
      plFree2dGrid(vec[fn][1],NMESH_X_TOTAL,NMESH_Z_TOTAL);
    }else if(strcmp(argv[1],"Z")==0) {
      view_vec(vec[fn], NMESH_X_TOTAL/VS, NMESH_Y_TOTAL/VS, 1, NMESH_X_TOTAL/VS, 1, NMESH_Y_TOTAL/VS,
	       "vec", "X", "Y");
      plFree2dGrid(vec[fn][0],NMESH_X_TOTAL,NMESH_Y_TOTAL);
      plFree2dGrid(vec[fn][1],NMESH_X_TOTAL,NMESH_Y_TOTAL);
    }
