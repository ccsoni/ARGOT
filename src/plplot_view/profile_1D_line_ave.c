...
int main(int argc, char **argv)
{
  ...
  for(int rank_x=0;rank_x<NNODE_X;rank_x++) {
    for(int rank_y=(NNODE_Y/2)-1; rank_y<(NNODE_Y/2)+1; rank_y++) {
      for(int rank_z=(NNODE_Z/2)-1; rank_z<(NNODE_Z/2)+1; rank_z++) {
	
	int iy, iz;
	if(rank_y==(NNODE_Y/2)-1) iy = NMESH_Y_LOCAL-1; //(NMESH_Y_TOTAL/2)-1;
	if(rank_y==(NNODE_Y/2))   iy = 0;               //(NMESH_Y_TOTAL/2);
	
	if(rank_z==(NNODE_Z/2)-1) iz = NMESH_Z_LOCAL-1; //(NMESH_Z_TOTAL/2)-1;
	if(rank_z==(NNODE_Z/2))   iz = 0;               //(NMESH_Z_TOTAL/2);
	
	FILE *fp;
    
	sprintf(filename, "%s_%03d_%03d_%03d", argv[1], rank_x, rank_y, rank_z);
