...
double calc_dtime(struct fluid_mesh *mesh, struct prim_chem *chem,
		  struct run_param *this_run)
{
#ifdef __UV_BACKGROUND__
  static float zred_prev=-1.0;

  static double Gamma_HI;
#ifdef __HELIUM__
  static double Gamma_HeI, Gamma_HeII;
#endif
#ifdef __HYDROGEN_MOL__
  static double Gamma_H2I_I,  Gamma_H2I_II;
  static double Gamma_H2II_I, Gamma_H2II_II, Gamma_HM;
#endif
...
#endif /* __HELIUM__ */
#ifdef __HYDROGEN_MOL__
  K_07=k07(T);  K_08=k08(T);
  K_09=k09(T);  K_10=k10(T);
  K_11=k11(T);  K_12=k12(T);
  K_13=k13(T);  K_14=k14(T);
  K_15=k15(T);  K_16=k16(T);
  K_17=k17(T);  K_18=k18(T);
  K_19=k19(T);  K_20=k20(T);
  K_21=k21(T);
...
void advance_reaction(struct fluid_mesh *mesh, struct prim_chem *chem,
		      struct run_param *this_run, float dtime)
{
  ...
  while(time_elapsed<dtime_p){
    ...
    // H2II; 
    //nH2II = (K_09*nHI*nHII+K_12*nH2I*nHII+K_19*nHM*nHII+Gamma_H2I_I*nH2I)/
    //(K_10*nHI + K_20*ne + K_21*nHM + (Gamma_H2II_I + Gamma_H2II_II));
    ...
    // H-;
    // nHM = (K_07*nHI*ne)/((K_08+K_17)*nHI+(K_18+K_19)*nHII+K_16*ne+Gamma_HM);
...
#else
    correct_fact = 1.0/(chem->fHI+chem->fHII+TINY);
#endif
...
void advance_reaction_and_heatcool(struct fluid_mesh *mesh, float *uene,
				   ...
				   int *nrec, int *niter)
{
  ...
  dtime_safe = DtFactor * calc_dtime(mesh, chem, this_run);
  ...
  while(time_elapsed<dtime_p){
    ...
    advance_heatcool(mesh, uene, chem, this_run, dtime_safe, &nrec, &niter);
    //    mesh->prev_uene = (*uene);
    ...
    K_01=k01(T); K_02=k02(T);
#ifdef __HELIUM__
    K_03=k03(T); K_04=k04(T); K_05=k05(T); K_06=k06(T);
#endif
#ifdef __HYDROGEN_MOL__
