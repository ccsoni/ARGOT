...
void setup_freq_param(struct freq_param *freq)
{
  ...
  freq->log_nu_min = -1.28; // 13.6*10^(-1.28) ~ 0.7 eV 
#else
  freq->log_nu_min = 0.0;
#endif
  freq->log_nu_max = 1.5; // 13.6*10^(1.5) ~ 430 eV
...
{
  double count_sum;
  double nu_lylimit, nu_max;

  nu_lylimit = 1.0 ; //pow(10.0, 0.0);
  nu_max     = pow(10.0, freq->log_nu_max);

  if(src->type==0) { /* blackbody */
    count_sum = count_photon(blackbody, (double) src->param, nu_lylimit, nu_max);
  }else if(src->type == 1) { /* power-law */
    count_sum = count_photon(power_law, (double) src->param, nu_lylimit, nu_max);
  }
